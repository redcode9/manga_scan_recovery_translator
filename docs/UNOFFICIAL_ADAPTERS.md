...
- **Batch capitoli**: `msrt run <reader-url> --all-chapters --i-own-rights` osserva la risposta reader `/ajax/read/<hid>/chapter/<lang>`, estrae i link capitolo, li ordina numericamente e lancia la stessa pipeline capitolo-per-capitolo. `--skip-existing` è attivo di default per riprendere batch lunghi senza rifare PDF già prodotti.
